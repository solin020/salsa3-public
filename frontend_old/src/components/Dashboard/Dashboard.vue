<template>
    <v-select 
        ...
        />
    <template v-if="ready">
        <j-tabs :tabnames="['overview', 'participants']">
                    ...
                    <thead>
                        <tr>
                            <th style="max-width: 20em; min-width: 20em;" :colspan="td_span">
                                ...
                                <br/>
                                Visits to date: {{ dashboard_total_summary?.total_due }}
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                                    ...
                                    <v-table >
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td v-for="p in phase_ids">
                                                    {{dashboard_phase_map?.get(p)?.total_complete}} ({{ dashboard_phase_map?.get(p)?.percent_complete }} %)
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td v-for="p in phase_ids">
                                                    {{dashboard_phase_map?.get(p)?.total_issues}} ({{ dashboard_phase_map?.get(p)?.percent_issues }} %)
                                                </td>
                                            </tr>
                                            <tr>
                                                <th>Not completed</th>
                                                ...
                                                <td v-for="p in phase_ids">
                                                    {{dashboard_phase_map?.get(p)?.total_missing}} ({{ dashboard_phase_map?.get(p)?.percent_missing }} %)
                                                </td>
                                            </tr>
                                        </tbody>
                                    </v-table>
                            ...
                            <td :colspan="td_span">
                                <details>
                                    ...
                                    <v-table>
                                        <phase-headers :phase_ids="phase_ids"/>
                                        <tbody>
                                            <template v-for="stim, i in unique_stimuli">
                                                <tr :class="{'border-top':i===0}"
                                                    >
                                                    <th style="max-width: 20em; min-width: 20em;"> {{ stim.id}} Submitted </th>
                                                    <td>
                                                    {{dashboard_total_stimulus_map?.get(stim.id)?.total_complete}}
                                                    ({{ dashboard_total_stimulus_map?.get(stim.id)?.percent_complete }}%)
                                                    </td>
                                                    <td v-for="phase_id in phase_ids">
                                                        {{dashboard_scheduled_per_phase_stimulus_map?.get(phase_id)?.get(stim.id)?.total_complete}}
                                                        ({{dashboard_scheduled_per_phase_stimulus_map?.get(phase_id)?.get(stim.id)?.percent_complete}}%)     
                                                    </td>
                                                </tr>
                                                <tr :class="{'border-top':i===0}"
                                                    >
                                                    <th> {{ stim.id }} Missing </th>
                                                    <td>
                                                    {{dashboard_total_stimulus_map?.get(stim.id)?.total_missing}}
                                                    ({{ dashboard_total_stimulus_map?.get(stim.id)?.percent_missing }}%)
                                                    </td>
                                                    <td v-for="phase_id in phase_ids">
                                                        {{dashboard_scheduled_per_phase_stimulus_map?.get(phase_id)?.get(stim.id)?.total_missing}}
                                                        ({{dashboard_scheduled_per_phase_stimulus_map?.get(phase_id)?.get(stim.id)?.percent_missing}}%)     
                                                    </td>
                                                </tr>
                                            </template>
                                        </tbody>
                        ...
                        <tr>
                            <td colspan=2>
                                <details>
                                    ...
                                    <v-table>
                                        <tbody>
                                            <tr v-for="stim, i in default_stimuli">
                                                <th style="max-width: 20em; min-width: 20em;"> Self initiated {{ stim.id }} </th>
                                                <td> {{ self_initiated_map.get(stim.id)?.count }}</td>
                                            </tr>
                                        </tbody>
                                    </v-table>
                                </details>
                ...
                <v-list>
                    <v-list-item>
                            <v-expansion-panels v-model="greater_panel">
                                <v-expansion-panel value="filters" title="Search by Filters">
                                ...
                                <v-expansion-panel value="participant_id" title="Search by login code">
                                    <v-expansion-panel-text>
                                        <v-text-field v-model="participant_id" label="Enter login code"></v-text-field>
                                    </v-expansion-panel-text>
                                </v-expansion-panel>
                    ...
                    <v-list-item v-if="participant_pages_ready">
                        <h2>Click on page of  {{participant_ids.length}} participant results to view:</h2>
                        ...
                        <v-table class="j-dashboard-table" v-if="participant_bubbles_ready && participant_pages_ready">
                            <thead>
                                <tr>
                                    ...
                                    <th></th>
                                    <th v-for="phase in current_schedule_prototype.phases">
                                        {{ phase.id }}
                                    </th>
                                    <th>
                                        Tests taken outside of schedule
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="pvis in all_participant_data">
                                    <th>
                                        ...
                                        <participant-overview 
                                            ...
                                            />
                                        <span  
                                         @click="download_participant_data(pvis.login_code)">
                                         &nbsp;&nbsp; ⬇️&nbsp;&nbsp; 
                                        </span>
                                    </th>
                                    ...
                                    <td>
                                        <v-btn @click="show_dropout_dialog(pvis.login_code)" >
                                            Dropout participant {{ pvis.login_code }}
                                        </v-btn>
                                    </td>
                                    <td v-for="phase in current_schedule_prototype.phases">
                                        <template v-for="test in phase.spec.tests!">
                                            <ViewVisit
                                                :my="pvis.visit_phase_sequence_map.get(`${phase.id}|${test.sequence_no}`)!"
                                            />
                                        </template>
                                    </td>
                                    <td>
...
import type { StimulusResult, Visit, ScheduledResponse, ScheduledVisit,SelfInitiatedResponse, SelfInitiatedVisit, Visit2, SubmissionQuality} from '@/types/schedule'
...
const phase_ids = computed(()=>current_schedule_prototype.value.phases.map(p => p.id))
...
const self_initiated_map = computed(()=>{
    const retval: Map<string,SelfInitiatedSummary[number]> = new Map()
    if (self_initiated_summary.value){
        for (const si of self_initiated_summary.value){
            retval.set(si.stimulus_id,  si)
        }
    }
    return retval
})
...
const dashboard_phase_map = computed(()=>{
    const retval : Map<string, DashboardSummary[number]> = new Map()
    if (dashboard_summary.value){
        for (const ds of dashboard_summary.value){
            if ( ds.phase_id){
                retval.set(ds.phase_id, ds)
            }
        }
    }
    return retval
...
const dashboard_scheduled_per_phase_stimulus_map = computed(()=>{
    
    const retval : Map<string, Map<string, PerStimulusSummary[number]>> = new Map()
    if (per_stimulus_summary.value){
        for (const pss of per_stimulus_summary.value){
            const phase_id = pss.phase_id
            if (phase_id){
                let phase_entry: Map<string, PerStimulusSummary[number]> | undefined = retval.get(phase_id)
                if (! phase_entry){
                    phase_entry = new Map()
                }
                phase_entry.set(pss.stimulus_id,pss)
                retval.set(phase_id, phase_entry)
            }
        }
    }
    return retval
...
const dashboard_total_stimulus_map = computed(()=>{
    const retval : Map<string, PerStimulusSummary[number]> = new Map()
    if (per_stimulus_summary.value){
        for (const pss of per_stimulus_summary.value){
            if (!pss.phase_id){
                retval.set(pss.stimulus_id, pss)
            }
        }
    }
    return retval
...
type ParticipantVisits = {
    ...
    self_initiated?: SelfInitiatedVisit[]
    phone_number:string,
    gamer:boolean,
...
type AllParticipantData = {
    visit_phase_sequence_map: Map<string, Visit2>
...
const participant_visits = ref<ParticipantVisits>([])
const all_participant_data = ref<AllParticipantData>([])

function calc_all_participant_data(pvv:ParticipantVisits):AllParticipantData{
    const retval: AllParticipantData = []
    for(const pv of pvv){
        const visit_phase_sequence_map = new Map<string, Visit2>()
        const visit_phase_sequence_stimulus_map = new Map<string, Visit2>()
        const self_initiated_stimulus_map:Map<string, Visit2>[]  = []
        const self_initiated: Visit2[] = []
        for (const ssv of pv.scheduled){
            const normal_visit:Visit2 = {
                participant_id: pv.participant_id,
                test_prototype_id: ssv.visit[0].test_prototype_id,
                formal_timestamp: ssv.scheduled_time,
                sequence_no: ssv.sequence_no,
                real_timestamp: ssv.real_timestamp,
                srs: ssv.visit,
                schedule_ids: [...new Set(ssv.visit.map(ssivis => ssivis.schedule_id))].filter(ssq => ssq !== 'future'),
                quality: ssv.status
            }
            visit_phase_sequence_map.set(`${ssv.phase_id}|${ssv.sequence_no}`, normal_visit)

            for (const stimid of unique_stimuli.value){
                const stimulus_quality = ((ssivi, stimid:string) => {
                    const relevant_stimuli = ssivi.visit.filter((ssivis) => 
                        ssivis.stimulus_id === stimid && ssivis.schedule_id !== 'future'
                    )
                    if (relevant_stimuli.length === 0){
                        return ssv.status
                    } else if (relevant_stimuli.length > 1){
                        return 'multiple' as SubmissionQuality
                    } else{
                        return 'perfect' as SubmissionQuality
                    }
                })(ssv, stimid.id)
                const sub_visit:Visit2 = {
                    participant_id: pv.participant_id,
                    test_prototype_id: ssv.visit[0].test_prototype_id,
                    formal_timestamp: ssv.scheduled_time,
                    sequence_no: ssv.sequence_no,
                ...
                visit_phase_sequence_stimulus_map.set(`${ssv.phase_id}|${ssv.sequence_no}|${stimid.id}`, sub_visit)

            }

        }
        if (pv.self_initiated){
            for (const ssiv of pv.self_initiated){
                const new_map = new Map<string, Visit2>()
                const new_visit:Visit2 = {
                    participant_id: pv.participant_id,
                    test_prototype_id: ssiv.visit[0].test_prototype_id,
                    formal_timestamp: ssiv.visit[0].real_timestamp!,
                    sequence_no: -1,
                    real_timestamp: ssiv.visit[0].real_timestamp,
                    srs: ssiv.visit,
                    schedule_ids: [...new Set(ssiv.visit.map(ssivis => ssivis.schedule_id))] ,
                    quality: 'perfect'
                }
                self_initiated.push( new_visit)
                for (const ssivis of ssiv.visit){
                    new_map.set(ssivis.schedule_id, new_visit)
                }
            }
        }
        const this_participant: AllParticipantData[number] = {
            visit_phase_sequence_map,
            visit_phase_sequence_stimulus_map,
            login_code: pv.participant_id,
            dashboard_code: pv.dashboard_code,
            downloaded: pv.downloaded,
            dropout: pv.dropout,
            dropout_reason: pv.dropout_reason,
            gamer: pv.gamer,
            gamer_reason: pv.gamer_reason,
            phone_number: pv.phone_number,
            self_initiated_stimulus_map,
            self_initiated,
        }
        retval.push(this_participant)
    }
    participant_bubbles_ready.value = true
    console.log('retvallq', current_schedule_prototype.value)
    return retval
}
...
async function get_participant_page_data(page:number){
    ...
    await fetch('/salsa/server/api/get-dashboard2-data?' + new URLSearchParams({
        ...
        s => {
            participant_visits.value = s
            active_page.value = page
            all_participant_data.value = calc_all_participant_data(participant_visits.value)
            participant_bubbles_ready.value = true
        }
    )
}
