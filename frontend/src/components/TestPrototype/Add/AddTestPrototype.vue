<template>
    <v-combobox
        v-model="test_prototype_type" :items="['phone', 'app']"
        label="Will this test prototype be administered over the phone or using the SALSA app?"
        :disabled="props.edit_existing"
    />
    <j-breadcrumbs v-if="test_prototype_type === 'app'"
    ...
    <j-breadcrumbs v-else-if="test_prototype_type==='phone'"
        :tabnames="['metadata', 'dialogtree-editor', 'submit']"
        :enableds="enableds"
    >
        <template #header_metadata>Configuration</template>
        <template #content_metadata>
                ...
                <v-row>
                    <v-col cols="12" lg="6">
                        <v-card style="height:100%">
                            ...
                            <v-text-field
                                v-model="my.id" label="Enter test prototype name" :rules="[id_regex_test, unique_test_prototype_id]"/>
                        </v-card>
                    </v-col>
                    <v-col cols="12" lg="6">
                        <v-card style="height:100%">
        ...
        <template #header_dialogtree-editor>Dialogtree Editor</template>
        <template #content_dialogtree-editor>
            <dialog-tree-outer :docname="first_id" :xmlstring="get_xmlstring()"/>
        </template>
        <template #header_submit>Submit Dialogtree</template>
        <template #content_submit>
            <j-btn  :click="submit_dialogtree" :arg0="null" :disabled="xmlstring===''">Upload dialogtree test prototype {{ my.id }}</j-btn>
        </template>
    </j-breadcrumbs>
</template>
...
import DialogTreeOuter from './DialogTreeOuter.vue'
...
const test_prototype_type = ref<'app'|'phone'>(props.my?.schedule_type||'app')
console.log('test_prototype_type', props.my?.schedule_type)
const first_id = props.my?.id ? `${props.my?.id}` : undefined


let my:TestPrototypeType = reactive({id:'', stimuli:[],  groups:[], randomized:false, schedule_type:test_prototype_type.value})
if (props.my){
    my = props.my
}
function reset(){
...
async function submit_dialogtree(){
